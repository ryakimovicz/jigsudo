...
import { initHome } from "./home.js?v=1.1.10";
import { initLanguage } from "./i18n.js?v=1.1.10";
import { initSudoku } from "./sudoku.js?v=1.1.10";
import { initHistory } from "./history.js?v=1.1.10";
import { initGuide, openTutorialModal } from "./guide.js?v=1.1.10"; // Guide & Tutorial Import
...
import {
  ...
  logoutUser,
  loginWithGoogle,
} from "./auth.js?v=1.1.10"; // Auth Import
import { initProfile, showProfile } from "./profile.js?v=1.1.10"; // Profile Import
import { CONFIG } from "./config.js?v=1.1.10"; // Keep CONFIG for displayVersion
import { router } from "./router.js?v=1.1.10"; // Router Import
import { closeSidebar, initSidebar } from "./sidebar.js?v=1.1.10";
import { initChangelog } from "./changelog.js?v=1.1.10";
import { toggleModal } from "./ui.js?v=1.1.10";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.1.10";
...
async function checkForUpdates() {
  try {
    ...
    if (serverVersion && isNewerVersion(serverVersion, localVersion)) {
      ...
      const { showUpdateToast } = await import("./ui.js?v=1.1.10");
...
async function startApp() {
  ...
  if (CONFIG.debugMode) {
    ...
    window.resetAccount = async () => {
      const { getCurrentUser } = await import("./auth.js?v=1.1.10");
      const { wipeUserData } = await import("./db.js?v=1.1.10");
    ...
    window.magicWand = async () => {
      const { debugAutoMatch } = await import("./memory.js?v=1.1.10");
      debugAutoMatch();
    };
...
function attachAuthListeners() {
  ...
  formRegister?.addEventListener("submit", async (e) => {
    ...
    if (pass !== confirmPass) {
      const { translations } = await import("./translations.js?v=1.1.10");
      const { getCurrentLang } = await import("./i18n.js?v=1.1.10");
  ...
  window.shareApp = async function () {
    ...
    try {
      if (navigator.share) {
        ...
        const { showToast } = await import("./ui.js?v=1.1.10");
