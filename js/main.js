...
import { initHome } from "./home.js?v=1.4.15";
import { initCookieConsent } from "./cookie-manager.js?v=1.4.15";
import { initLanguage } from "./i18n.js?v=1.4.15";
import { initSudoku } from "./sudoku.js?v=1.4.15";
import { initHistory } from "./history.js?v=1.4.15";
import { initGuide, openTutorialModal } from "./guide.js?v=1.4.15"; // Guide & Tutorial Import
...
import {
  ...
  logoutUser,
  loginWithGoogle,
} from "./auth.js?v=1.4.15"; // Auth Import
import { initProfile, showProfile } from "./profile.js?v=1.4.15"; // Profile Import
import { CONFIG } from "./config.js?v=1.4.15"; // Keep CONFIG for displayVersion
import { router } from "./router.js?v=1.4.15"; // Router Import
import { closeSidebar, initSidebar } from "./sidebar.js?v=1.4.15";
import { initChangelog } from "./changelog.js?v=1.4.15";
import { toggleModal } from "./ui.js?v=1.4.15";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.4.15";
import { checkSeasonMigration } from "./migration.js?v=1.4.15";
import { initSearchUsers } from "./search-users.js?v=1.4.15";
...
async function checkForUpdates() {
  try {
    ...
    if (serverVersion && isNewerVersion(serverVersion, localVersion)) {
      ...
      const { showUpdateToast } = await import("./ui.js?v=1.4.15");
...
async function startApp() {
  ...
  if (CONFIG.debugMode) {
    ...
    window.resetAccount = async () => {
      const { getCurrentUser, logoutUser } = await import("./auth.js?v=1.4.15");
      const { wipeUserData } = await import("./db.js?v=1.4.15");
    ...
    window.magicWand = async () => {
      const { debugAutoMatch } = await import("./memory.js?v=1.4.15");
