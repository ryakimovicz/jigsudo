...
import { initHome } from "./home.js?v=1.4.21";
import { initCookieConsent } from "./cookie-manager.js?v=1.4.21";
import { initLanguage } from "./i18n.js?v=1.4.21";
import { initSudoku } from "./sudoku.js?v=1.4.21";
import { initHistory } from "./history.js?v=1.4.21";
import { initGuide, openTutorialModal } from "./guide.js?v=1.4.21"; // Guide & Tutorial Import
import { gameManager } from "./game-manager.js?v=1.4.21";
import {
  initAuth,
  loginUser,
  registerUser,
  logoutUser,
  loginWithGoogle,
} from "./auth.js?v=1.4.21"; // Auth Import
import { initProfile, showProfile } from "./profile.js?v=1.4.21"; // Profile Import
import { CONFIG } from "./config.js?v=1.4.21"; // Keep CONFIG for displayVersion
import { router } from "./router.js?v=1.4.21"; // Router Import
import { closeSidebar, initSidebar } from "./sidebar.js?v=1.4.21";
import { initChangelog } from "./changelog.js?v=1.4.21";
import { toggleModal } from "./ui.js?v=1.4.21";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.4.21";
import { checkSeasonMigration } from "./migration.js?v=1.4.21";
import { initSearchUsers } from "./search-users.js?v=1.4.21";
...
async function checkForUpdates() {
  try {
    ...
    if (serverVersion && isNewerVersion(serverVersion, localVersion)) {
      ...
      const { showUpdateToast } = await import("./ui.js?v=1.4.21");
...
async function startApp() {
  ...
  if (CONFIG.debugMode) {
    ...
    window.resetAccount = async () => {
      const { getCurrentUser, logoutUser } = await import("./auth.js?v=1.4.21");
      const { wipeUserData } = await import("./db.js?v=1.4.21");
    ...
    window.magicWand = async () => {
      const { debugAutoMatch } = await import("./memory.js?v=1.4.21");
