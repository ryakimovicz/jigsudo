...
import { initHome } from "./home.js?v=1.1.17";
import { initLanguage } from "./i18n.js?v=1.1.17";
import { initSudoku } from "./sudoku.js?v=1.1.17";
import { initHistory } from "./history.js?v=1.1.17";
import { initGuide, openTutorialModal } from "./guide.js?v=1.1.17"; // Guide & Tutorial Import
...
import {
  ...
  logoutUser,
  loginWithGoogle,
} from "./auth.js?v=1.1.17"; // Auth Import
import { initProfile, showProfile } from "./profile.js?v=1.1.17"; // Profile Import
import { CONFIG } from "./config.js?v=1.1.17"; // Keep CONFIG for displayVersion
import { router } from "./router.js?v=1.1.17"; // Router Import
import { closeSidebar, initSidebar } from "./sidebar.js?v=1.1.17";
import { initChangelog } from "./changelog.js?v=1.1.17";
import { toggleModal } from "./ui.js?v=1.1.17";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.1.17";
...
async function checkForUpdates() {
  try {
    ...
    if (serverVersion && isNewerVersion(serverVersion, localVersion)) {
      ...
      const { showUpdateToast } = await import("./ui.js?v=1.1.17");
...
async function startApp() {
  ...
  if (CONFIG.debugMode) {
    ...
    window.resetAccount = async () => {
      const { getCurrentUser } = await import("./auth.js?v=1.1.17");
      const { wipeUserData } = await import("./db.js?v=1.1.17");
    ...
    window.magicWand = async () => {
      const { debugAutoMatch } = await import("./memory.js?v=1.1.17");
      debugAutoMatch();
    };
...
function attachAuthListeners() {
  ...
  formRegister?.addEventListener("submit", async (e) => {
    ...
    if (pass !== confirmPass) {
      const { translations } = await import("./translations.js?v=1.1.17");
      const { getCurrentLang } = await import("./i18n.js?v=1.1.17");
  ...
  window.shareApp = async function () {
    ...
    try {
      if (navigator.share) {
        ...
        const { showToast } = await import("./ui.js?v=1.1.17");
