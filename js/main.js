...
import { initHome } from "./home.js?v=1.1.16";
import { initLanguage } from "./i18n.js?v=1.1.16";
import { initSudoku } from "./sudoku.js?v=1.1.16";
import { initHistory } from "./history.js?v=1.1.16";
import { initGuide, openTutorialModal } from "./guide.js?v=1.1.16"; // Guide & Tutorial Import
...
import {
  ...
  logoutUser,
  loginWithGoogle,
} from "./auth.js?v=1.1.16"; // Auth Import
import { initProfile, showProfile } from "./profile.js?v=1.1.16"; // Profile Import
import { CONFIG } from "./config.js?v=1.1.16"; // Keep CONFIG for displayVersion
import { router } from "./router.js?v=1.1.16"; // Router Import
import { closeSidebar, initSidebar } from "./sidebar.js?v=1.1.16";
import { initChangelog } from "./changelog.js?v=1.1.16";
import { toggleModal } from "./ui.js?v=1.1.16";
...
async function checkForUpdates() {
  try {
    ...
    if (serverVersion && isNewerVersion(serverVersion, localVersion)) {
      ...
      const { showUpdateToast } = await import("./ui.js?v=1.1.16");
...
async function startApp() {
  ...
  if (CONFIG.debugMode) {
    ...
    window.resetAccount = async () => {
      ...
      const { wipeUserData } = await import("./db.js?v=1.1.16");
    ...
    window.magicWand = async () => {
      const { debugAutoMatch } = await import("./memory.js?v=1.1.16");
      debugAutoMatch();
    };
