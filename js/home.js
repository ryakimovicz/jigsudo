...
import { isPuzzleAvailable } from "./history.js?v=1.1.16";
import { showAlertModal } from "./ui.js?v=1.1.16";
import { getJigsudoDate } from "./utils/time.js?v=1.1.16";
...
export function initHome() {
  ...
  if (confirmToggle) {
    ...
    confirmToggle.addEventListener("change", async () => {
      ...
      const { updateUserPreference } = await import("./db.js?v=1.1.16");
  ...
  const handleViewResults = async () => {
    try {
      ...
      if (todayStats) {
        ...
        const { showVictorySummary } = await import("./ui.js?v=1.1.16");
  ...
  if (navHome) {
    navHome.addEventListener("click", async () => {
      ...
      const { router } = await import("./router.js?v=1.1.16");
  ...
  window.addEventListener("gameCompleted", async () => {
    ...
    const { clearRankingCache } = await import("./ranking.js?v=1.1.16");
    clearRankingCache();
...
export async function startDailyGame() {
  ...
  try {
    ...
    const module = await import("./memory.js?v=1.1.16");
