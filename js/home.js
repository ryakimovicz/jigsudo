...
import { getCurrentLang } from "./i18n.js?v=1.2.0";
import { showProfile } from "./profile.js?v=1.2.0";
import { getDailySeed } from "./utils/random.js?v=1.2.0";
import { gameManager } from "./game-manager.js?v=1.2.0";
import { fetchRankings, renderRankings, clearRankingCache, getCachedRankings } from "./ranking.js?v=1.2.0";
import { getCurrentUser } from "./auth.js?v=1.2.0";
import { CONFIG } from "./config.js?v=1.2.0";
import { updateSidebarActiveState } from "./sidebar.js?v=1.2.0";
import { router } from "./router.js?v=1.2.0";
import { isPuzzleAvailable } from "./history.js?v=1.2.0";
import { showAlertModal } from "./ui.js?v=1.2.0";
import { getJigsudoDate } from "./utils/time.js?v=1.2.0";
...
export function initHome() {
  ...
  if (confirmToggle) {
    ...
    confirmToggle.addEventListener("change", async () => {
      ...
      const { updateUserPreference } = await import("./db.js?v=1.2.0");
  ...
  const handleViewResults = async () => {
    try {
      ...
      if (todayStats) {
        ...
        const { showVictorySummary } = await import("./ui.js?v=1.2.0");
  ...
  if (navHome) {
    navHome.addEventListener("click", async () => {
      ...
      const { router } = await import("./router.js?v=1.2.0");
  ...
  window.addEventListener("gameCompleted", async () => {
    ...
    const { clearRankingCache } = await import("./ranking.js?v=1.2.0");
    clearRankingCache();
...
export async function startDailyGame() {
  ...
  try {
    ...
    const module = await import("./memory.js?v=1.2.0");
