...
import { showProfile } from "./profile.js?v=1.4.2";
import { getDailySeed } from "./utils/random.js?v=1.4.2";
import { gameManager } from "./game-manager.js?v=1.4.2";
import { fetchRankings, renderRankings, clearRankingCache, getCachedRankings } from "./ranking.js?v=1.4.2";
import { getCurrentUser } from "./auth.js?v=1.4.2";
import { CONFIG } from "./config.js?v=1.4.2";
import { updateSidebarActiveState } from "./sidebar.js?v=1.4.2";
import { router } from "./router.js?v=1.4.2";
import { isPuzzleAvailable } from "./history.js?v=1.4.2";
import { showAlertModal } from "./ui.js?v=1.4.2";
import { getJigsudoDate, getJigsudoDateString } from "./utils/time.js?v=1.4.2";
...
export function initHome() {
  ...
  if (confirmToggle) {
    ...
    confirmToggle.addEventListener("change", async () => {
      ...
      const { updateUserPreference } = await import("./db.js?v=1.4.2");
  ...
  const handleViewResults = async () => {
    try {
      ...
      if (todayStats) {
        ...
        const { showVictorySummary } = await import("./ui.js?v=1.4.2");
  ...
  if (navHome) {
    navHome.addEventListener("click", async () => {
      ...
      const { router } = await import("./router.js?v=1.4.2");
  ...
  async function loadAndRenderAllRankings(force = false) {
    ...
    const { getCachedRankings, renderRankings, fetchRankings, clearRankingCache } = await import("./ranking.js?v=1.4.2");
  ...
  window.addEventListener("gameCompleted", async (e) => {
    ...
    const { clearRankingCache } = await import("./ranking.js?v=1.4.2");
    clearRankingCache();
...
export async function startDailyGame() {
  ...
  try {
    ...
    const module = await import("./memory.js?v=1.4.2");
