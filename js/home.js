...
import { showProfile } from "./profile.js?v=1.3.8";
import { getDailySeed } from "./utils/random.js?v=1.3.8";
import { gameManager } from "./game-manager.js?v=1.3.8";
import { fetchRankings, renderRankings, clearRankingCache, getCachedRankings } from "./ranking.js?v=1.3.8";
import { getCurrentUser } from "./auth.js?v=1.3.8";
import { CONFIG } from "./config.js?v=1.3.8";
import { updateSidebarActiveState } from "./sidebar.js?v=1.3.8";
import { router } from "./router.js?v=1.3.8";
import { isPuzzleAvailable } from "./history.js?v=1.3.8";
import { showAlertModal } from "./ui.js?v=1.3.8";
import { getJigsudoDate, getJigsudoDateString } from "./utils/time.js?v=1.3.8";
...
export function initHome() {
  ...
  if (confirmToggle) {
    ...
    confirmToggle.addEventListener("change", async () => {
      ...
      const { updateUserPreference } = await import("./db.js?v=1.3.8");
  ...
  const handleViewResults = async () => {
    try {
      ...
      if (todayStats) {
        ...
        const { showVictorySummary } = await import("./ui.js?v=1.3.8");
  ...
  if (navHome) {
    navHome.addEventListener("click", async () => {
      ...
      const { router } = await import("./router.js?v=1.3.8");
  ...
  async function loadAndRenderAllRankings(force = false) {
    ...
    const { getCachedRankings, renderRankings, fetchRankings, clearRankingCache } = await import("./ranking.js?v=1.3.8");
  ...
  window.addEventListener("gameCompleted", async (e) => {
    ...
    const { clearRankingCache } = await import("./ranking.js?v=1.3.8");
    clearRankingCache();
...
export async function startDailyGame() {
  ...
  try {
    ...
    const module = await import("./memory.js?v=1.3.8");
