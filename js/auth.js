...
import { auth } from "./firebase-config.js?v=1.1.6";
import {
...
import { router } from "./router.js?v=1.1.6";
import { translations } from "./translations.js?v=1.1.6";
import { getCurrentLang } from "./i18n.js?v=1.1.6";
import { toggleModal, showToast } from "./ui.js?v=1.1.6";
    ...
    if (user) {
      ...
      try {
        ...
        if (
          ...
          const memoryModule = await import("./memory.js?v=1.1.6");
...
export async function logoutUser() {
  try {
    const { stopListeningAndCleanup } = await import("./db.js?v=1.1.6");
...
export async function loginWithGoogle() {
  ...
  try {
    ...
    const { db } = await import("./firebase-config.js?v=1.1.6");
...
function updateUIForLogin(user) {
  ...
  if (!isNavigating && !isGameActive) {
    if (window.location.hash.startsWith("#profile")) {
      ...
      import("./profile.js?v=1.1.6").then((mod) => {
  ...
  import("./profile.js?v=1.1.6").then((module) => {
  ...
  if (privacyToggle) {
    import("./db.js?v=1.1.6").then(async (dbMod) => {
...
function updateUIForLogout() {
  ...
  import("./db.js?v=1.1.6").then((module) => {
  ...
  if (actionType === "change_username") {
    ...
    newBtnConfirm.onclick = async () => {
      ...
      if (result.success) {
        ...
        try {
          const { updateProfileData } = await import("./profile.js?v=1.1.6");
          updateProfileData();
        } catch (e) {
