...
import { auth } from "./firebase-config.js?v=1.4.20";
import {
...
import { toggleModal, showToast } from "./ui.js?v=1.4.20";
    ...
    if (user) {
      ...
      try {
        ...
        if (isAtGame && !gameManager.isReplaying) {
          ...
          const memoryModule = await import("./memory.js?v=1.4.20");
...
export async function logoutUser() {
  try {
    const { stopListeningAndCleanup } = await import("./db.js?v=1.4.20");
...
function updateUIForLogin(user) {
  ...
  if (!isNavigating && !isGameActive) {
    if (window.location.hash.startsWith("#profile")) {
      ...
      import("./profile.js?v=1.4.20").then((mod) => {
  ...
  import("./profile.js?v=1.4.20").then((module) => {
  ...
  if (privacyToggle) {
    ...
    import("./db.js?v=1.4.20").then(async (dbMod) => {
...
function updateUIForLogout() {
  ...
  import("./db.js?v=1.4.20").then((module) => {
  ...
  if (actionType === "change_username") {
    ...
    newBtnConfirm.onclick = async () => {
      ...
      if (result.success) {
        ...
        try {
          const { updateProfileData } = await import("./profile.js?v=1.4.20");
          updateProfileData();
        } catch (e) {
