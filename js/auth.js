...
import { auth } from "./firebase-config.js?v=1.4.16";
import {
...
import { toggleModal, showToast } from "./ui.js?v=1.4.16";
    ...
    if (user) {
      ...
      try {
        ...
        if (isAtGame && !gameManager.isReplaying) {
          ...
          const memoryModule = await import("./memory.js?v=1.4.16");
...
export async function logoutUser() {
  try {
    const { stopListeningAndCleanup } = await import("./db.js?v=1.4.16");
...
function updateUIForLogin(user) {
  ...
  if (!isNavigating && !isGameActive) {
    if (window.location.hash.startsWith("#profile")) {
      ...
      import("./profile.js?v=1.4.16").then((mod) => {
  ...
  import("./profile.js?v=1.4.16").then((module) => {
  ...
  if (privacyToggle) {
    ...
    import("./db.js?v=1.4.16").then(async (dbMod) => {
...
function updateUIForLogout() {
  ...
  import("./db.js?v=1.4.16").then((module) => {
  ...
  if (actionType === "change_username") {
    ...
    newBtnConfirm.onclick = async () => {
      ...
      if (result.success) {
        ...
        try {
          const { updateProfileData } = await import("./profile.js?v=1.4.16");
          updateProfileData();
        } catch (e) {
