...
import { showToast, updateLevelTitle, updateGameHelp } from "./ui.js?v=1.4.7";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.4.7";
import { resetUI } from "./memory.js?v=1.4.7";
import { initCode } from "./code.js?v=1.4.7";
...
export async function transitionToSearch() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.4.7");
  const { resumePeaksState } = await import("./peaks.js?v=1.4.7");
  resumeSudokuState();
  resumePeaksState();
}
...
export async function transitionToCode() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.4.7");
  const { resumePeaksState } = await import("./peaks.js?v=1.4.7");
  const { resumeCodeState } = await import("./code.js?v=1.4.7");
