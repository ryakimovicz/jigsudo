...
import { resetUI } from "./memory.js?v=1.3.0";
import { initCode } from "./code.js?v=1.3.0";
...
export async function transitionToSearch() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.3.0");
  const { resumePeaksState } = await import("./peaks.js?v=1.3.0");
  resumeSudokuState();
  resumePeaksState();
}
...
export async function transitionToCode() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.3.0");
  const { resumePeaksState } = await import("./peaks.js?v=1.3.0");
  const { resumeCodeState } = await import("./code.js?v=1.3.0");
