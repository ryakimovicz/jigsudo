...
import { showToast } from "./ui.js?v=1.5.30";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.5.30";
import { resetUI } from "./memory.js?v=1.5.30";
import { initCode } from "./code.js?v=1.5.30";
...
export async function transitionToSearch() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.5.30");
  const { resumePeaksState } = await import("./peaks.js?v=1.5.30");
  resumeSudokuState();
  resumePeaksState();
}
...
export async function transitionToCode() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.5.30");
  const { resumePeaksState } = await import("./peaks.js?v=1.5.30");
  const { resumeCodeState } = await import("./code.js?v=1.5.30");
