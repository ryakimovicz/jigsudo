...
import { showToast } from "./ui.js?v=1.5.55";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.5.55";
import { resetUI } from "./memory.js?v=1.5.55";
import { initCode } from "./code.js?v=1.5.55";
...
export async function transitionToSearch() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.5.55");
  const { resumePeaksState } = await import("./peaks.js?v=1.5.55");
  resumeSudokuState();
  resumePeaksState();
}
...
export async function transitionToCode() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.5.55");
  const { resumePeaksState } = await import("./peaks.js?v=1.5.55");
  const { resumeCodeState } = await import("./code.js?v=1.5.55");
