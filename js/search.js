...
import { isPeakOrValley, getNeighbors } from "./peaks-logic.js?v=1.1.14";
import { initCode } from "./code.js?v=1.1.14";
import { resetUI } from "./memory.js?v=1.1.14";
...
export async function transitionToSearch() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.1.14");
  const { resumePeaksState } = await import("./peaks.js?v=1.1.14");
  resumeSudokuState();
  resumePeaksState();
}
...
export async function transitionToCode() {
  ...
  const { resumeSudokuState } = await import("./sudoku.js?v=1.1.14");
  const { resumePeaksState } = await import("./peaks.js?v=1.1.14");
  const { resumeCodeState } = await import("./code.js?v=1.1.14");
