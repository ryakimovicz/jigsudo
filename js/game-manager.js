import { getDailySeed } from "./utils/random.js?v=1.4.17";
// Local generation removed per user request (Cloud Only)
import {
  generateSearchSequences,
  countSequenceOccurrences,
} from "./search-gen.js?v=1.4.17";
import { CONFIG } from "./config.js?v=1.4.17";
import { calculateRP, getRankData, SCORING } from "./ranks.js?v=1.4.17";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.4.17";
import { encryptData, decryptData } from "./utils/crypto.js?v=1.4.17";
import { getJigsudoDateString, getJigsudoYearMonth, getJigsudoDayDiff } from "./utils/time.js?v=1.4.17";
import { masterLock } from "./lock.js?v=1.4.17";
...
export class GameManager {
  ...
  async recordStart() {
    ...
    const { getCurrentUser } = await import("./auth.js?v=1.4.17");
    const user = getCurrentUser();

    if (user && !user.isAnonymous) {
      console.log("[Referee] Registering game start and maintenance check on server...");
      try {
        const { callJigsudoFunction } = await import("./db.js?v=1.4.17");
    ...
    if (user && !user.isAnonymous) {
      import("./db.js?v=1.4.17").then(m => m.initializeHistoryDocument(user.uid, this.currentSeed));
    } else {
      // Local Guest History
  ...
  async ensureSearchTargets() {
    ...
    try {
      ...
      const { generateSearchSequences } = await import("./search-gen.js?v=1.4.17");
  ...
  async forceCloudSave(isWonNow = false, isPenalty = false, isReset = false) {
    ...
    try {
      const { getCurrentUser } = await import("./auth.js?v=1.4.17");
      const { saveUserProgress, saveUserStats } = await import("./db.js?v=1.4.17");
  ...
  async resetCurrentGame() {
    ...
    try {
      const { getCurrentUser } = await import("./auth.js?v=1.4.17");
      const { showNotification } = await import("./ui.js?v=1.4.17");
      const user = getCurrentUser();
      ...
      if (user && !user.isAnonymous) {
        ...
        const { deleteHistoryEntry } = await import("./db.js?v=1.4.17");
  ...
  async _processValidationQueue() {
    ...
    const { getCurrentUser } = await import("./auth.js?v=1.4.17");
    const { saveUserStats } = await import("./db.js?v=1.4.17");
    ...
    while (this.validationQueue.length > 0) {
      ...
      try {
        ...
        if (result.data.status === "success" || result.data.status === "already_verified") {
          ...
          const { sendRefereeReport } = await import("./db.js?v=1.4.17");
          const { getCurrentUser } = await import("./auth.js?v=1.4.17");
          const user = getCurrentUser();
  ...
  async _recalculateNetStats(isPenalty = false, skipPersistence = false) {
    ...
    const { getDateStringFromSeed } = await import("./utils/time.js?v=1.4.17");
  ...
  async _checkRankDecay(targetStats = null) {
    ...
    if (lastCheck && lastCheck !== today) {
      ...
      const currDate = new Date(today + "T12:00:00Z");
        ...
        const { getRankData } = await import("./ranks.js?v=1.4.17");
  ...
  async ensureSessionStarted(markIntent = false) {
    ...
    if (user && !user.isAnonymous) {
        try {
            ...
            const { db } = await import("./firebase-config.js?v=1.4.17");
  ...
  async handleCloudSync(remoteProgress, remoteStats, isSnapshot = false, remoteSettings = null) {
    ...
    const { isAtGameRoute } = await import("./utils/route-utils.js?v=1.4.17");
  ...
  showConflictResolution(remoteState) {
    ...
    if (btnKeep) {
      btnKeep.onclick = async () => {
        ...
        await this.forceCloudSave();
        // 4. (Optional) Toast
        const { showToast } = await import("./ui.js?v=1.4.17");
  ...
  async showExclusiveSessionBlock() {
    ...
    try {
      ...
      const { translations: tData } = await import("./translations.js?v=1.4.17");
      if (tData && tData[lang]) {
        t = { ...t, ...tData[lang] };
      }
  ...
  async recordWin() {
    ...
    return this.enqueueStatsUpdate(async () => {
      ...
      const { getCurrentUser } = await import("./auth.js?v=1.4.17");
      const { calculateTimeBonus } = await import("./ranks.js?v=1.4.17");
      const { callJigsudoFunction, saveUserStats, saveHistoryEntry } = await import("./db.js?v=1.4.17");
      ...
      if (!this.isReplay && !isAlreadyWon) {
        if (user && !user.isAnonymous) {
            ...
            if (this.state) {
                ...
                const { saveUserProgress } = await import("./db.js?v=1.4.17");
      ...
      // --- 5. CLOUD SYNC ---
      if (user && !user.isAnonymous) {
        const { getJigsudoDateString, getJigsudoYearMonth } = await import("./utils/time.js?v=1.4.17");
      ...
      // --- 6. UX & NOTIFICATION ---
      const { stopTimer } = await import("./timer.js?v=1.4.17");
      const { showToast } = await import("./ui.js?v=1.4.17");
      const { getCurrentLang } = await import("./i18n.js?v=1.4.17");
      const { translations } = await import("./translations.js?v=1.4.17");
