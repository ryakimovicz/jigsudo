...
import { calculateRP, getRankData, SCORING } from "./ranks.js?v=1.4.6";
import { isAtGameRoute } from "./utils/route-utils.js?v=1.4.6";
import { encryptData, decryptData } from "./utils/crypto.js?v=1.4.6";
import { getJigsudoDateString, getJigsudoYearMonth, getJigsudoDayDiff } from "./utils/time.js?v=1.4.6";
import { masterLock } from "./lock.js?v=1.4.6";
...
export class GameManager {
  ...
  async resetCurrentGame() {
    ...
    try {
      ...
      if (user && !user.isAnonymous) {
        ...
        const { deleteHistoryEntry } = await import("./db.js?v=1.4.6");
    ...
    while (this.validationQueue.length > 0) {
      ...
      try {
        ...
        if (result.data.status === "success" || result.data.status === "already_verified") {
          ...
          const { sendRefereeReport } = await import("./db.js?v=1.4.6");
          const { getCurrentUser } = await import("./auth.js?v=1.4.6");
          const user = getCurrentUser();
  ...
  async _recalculateNetStats(isPenalty = false, skipPersistence = false) {
    ...
    const { getDateStringFromSeed } = await import("./utils/time.js?v=1.4.6");
  ...
  async ensureSessionStarted() {
    ...
    if (user && !user.isAnonymous) {
        try {
            ...
            const { db } = await import("./firebase-config.js?v=1.4.6");
  ...
  async handleCloudSync(remoteProgress, remoteStats, isSnapshot = false, remoteSettings = null) {
    ...
    const { isAtGameRoute } = await import("./utils/route-utils.js?v=1.4.6");
  ...
  async recordWin() {
    ...
    return this.enqueueStatsUpdate(async () => {
      ...
      const { callJigsudoFunction, saveUserStats, saveHistoryEntry } = await import("./db.js?v=1.4.6");
      ...
      if (!this.isReplay && !isAlreadyWon) {
        if (user && !user.isAnonymous) {
            ...
            if (this.state) {
                ...
                const { saveUserProgress } = await import("./db.js?v=1.4.6");
