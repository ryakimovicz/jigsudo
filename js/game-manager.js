...
} from "./search-gen.js?v=1.1.13";
import { CONFIG } from "./config.js?v=1.1.13";
import { calculateRP, SCORING } from "./ranks.js?v=1.1.13";
...
export class GameManager {
  ...
  async ensureSearchTargets() {
    ...
    try {
      ...
      const { generateSearchSequences } = await import("./search-gen.js?v=1.1.13");
  ...
  async forceCloudSave(overrideUid = null) {
    ...
    try {
      const { getCurrentUser } = await import("./auth.js?v=1.1.13");
      const { saveUserProgress, saveUserStats } = await import("./db.js?v=1.1.13");
  ...
  async resetCurrentGame() {
    ...
    try {
      const { getCurrentUser } = await import("./auth.js?v=1.1.13");
      const { saveUserProgress, saveUserStats } = await import("./db.js?v=1.1.13");
      const { showNotification } = await import("./ui.js?v=1.1.13");
      const user = getCurrentUser();
  ...
  async awardStagePoints(stage) {
    ...
    const { saveUserStats } = await import("./db.js?v=1.1.13");
    const { getCurrentUser } = await import("./auth.js?v=1.1.13");
    const { getJigsudoDateString, getJigsudoYearMonth } =
      await import("./utils/time.js?v=1.1.13");
    const user = getCurrentUser();
    if (user && !user.isAnonymous) {
      ...
      if (diffDays >= 1) {
        ...
        if (lastIntent && lastIntent !== today) {
          ...
          if (intentDiff > 1) {
            ...
            const { getRankData } = await import("./ranks.js?v=1.1.13");
  ...
  async recordWin() {
    ...
    try {
      ...
      if (!this.isReplay && !isAlreadyWon) {
        ...
        const peaksErrors = this.state.stats?.peaksErrors || 0;
        const { calculateTimeBonus } = await import("./ranks.js?v=1.1.13");
      ...
      const { saveUserStats } = await import("./db.js?v=1.1.13");
      const { stopTimer } = await import("./timer.js?v=1.1.13");
      stopTimer();
      const { getDailySeed } = await import("./utils/random.js?v=1.1.13");
      const user = await import("./auth.js?v=1.1.13").then((m) => m.getCurrentUser());
      if (user && !user.isAnonymous) {
        const { getJigsudoDateString, getJigsudoYearMonth } =
          await import("./utils/time.js?v=1.1.13");
        const today = getJigsudoDateString();
