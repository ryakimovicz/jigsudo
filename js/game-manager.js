...
} from "./search-gen.js?v=1.1.19";
import { CONFIG } from "./config.js?v=1.1.19";
import { calculateRP, SCORING } from "./ranks.js?v=1.1.19";
...
export class GameManager {
  ...
  async ensureSearchTargets() {
    ...
    try {
      ...
      const { generateSearchSequences } = await import("./search-gen.js?v=1.1.19");
      ...
      if (diffDays >= 1) {
        ...
        if (lastIntent && lastIntent !== today) {
          ...
          if (intentDiff > 1) {
            ...
            const { getRankData } = await import("./ranks.js?v=1.1.19");
  ...
  async recordWin() {
    ...
    try {
      ...
      if (!this.isReplay && !isAlreadyWon) {
        ...
        const peaksErrors = this.state.stats?.peaksErrors || 0;
        const { calculateTimeBonus } = await import("./ranks.js?v=1.1.19");
      ...
      const { saveUserStats } = await import("./db.js?v=1.1.19");
      const { stopTimer } = await import("./timer.js?v=1.1.19");
      stopTimer();
      const { getDailySeed } = await import("./utils/random.js?v=1.1.19");
      const user = await import("./auth.js?v=1.1.19").then((m) => m.getCurrentUser());
      if (user && !user.isAnonymous) {
