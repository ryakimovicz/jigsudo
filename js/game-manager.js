...
export class GameManager {
  ...
  async recordStart() {
    ...
    if (user && !user.isAnonymous) {
      ...
      try {
        const { callJigsudoFunction } = await import("./db.js?v=1.3.1");
    ...
    if (user && !user.isAnonymous) {
      import("./db.js?v=1.3.1").then(m => m.initializeHistoryDocument(user.uid, this.currentSeed));
    } else {
      // Local Guest History
  ...
  async forceCloudSave(overrideUid = null, isPenalty = false) {
    ...
    try {
      const { getCurrentUser } = await import("./auth.js?v=1.3.1");
      const { saveUserProgress, saveUserStats } = await import("./db.js?v=1.3.1");
  ...
  async resetCurrentGame() {
    ...
    try {
      const { getCurrentUser } = await import("./auth.js?v=1.3.1");
      const { showNotification } = await import("./ui.js?v=1.3.1");
      const user = getCurrentUser();
  ...
  async _processValidationQueue() {
    ...
    const { getCurrentUser } = await import("./auth.js?v=1.3.1");
    const { saveUserStats } = await import("./db.js?v=1.3.1");
    ...
    while (this.validationQueue.length > 0) {
      ...
      try {
        ...
        if (result.data.status === "success" || result.data.status === "already_verified") {
          ...
          const { sendRefereeReport } = await import("./db.js?v=1.3.1");
          const { getCurrentUser } = await import("./auth.js?v=1.3.1");
          const user = getCurrentUser();
  ...
  async recordWin() {
    ...
    try {
      ...
      const { callJigsudoFunction, saveUserStats, saveHistoryEntry } = await import("./db.js?v=1.3.1");
      ...
      // --- 5. CLOUD SYNC ---
      if (user && !user.isAnonymous) {
        const { getJigsudoDateString, getJigsudoYearMonth } = await import("./utils/time.js?v=1.3.1");
