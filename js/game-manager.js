...
export class GameManager {
  ...
  async ensureSearchTargets() {
    ...
    try {
      ...
      const { generateSearchSequences } = await import("./search-gen.js?v=1.1.14");
  ...
  async forceCloudSave(overrideUid = null) {
    ...
    try {
      const { getCurrentUser } = await import("./auth.js?v=1.1.14");
      const { saveUserProgress, saveUserStats } = await import("./db.js?v=1.1.14");
  ...
  async resetCurrentGame() {
    ...
    try {
      const { getCurrentUser } = await import("./auth.js?v=1.1.14");
      const { saveUserProgress, saveUserStats } = await import("./db.js?v=1.1.14");
      const { showNotification } = await import("./ui.js?v=1.1.14");
      const user = getCurrentUser();
  ...
  async awardStagePoints(stage) {
    ...
    const { saveUserStats } = await import("./db.js?v=1.1.14");
    const { getCurrentUser } = await import("./auth.js?v=1.1.14");
    const { getJigsudoDateString, getJigsudoYearMonth } =
      await import("./utils/time.js?v=1.1.14");
    const user = getCurrentUser();
    if (user && !user.isAnonymous) {
      ...
      if (diffDays >= 1) {
        ...
        if (lastIntent && lastIntent !== today) {
          ...
          if (intentDiff > 1) {
            ...
            const { getRankData } = await import("./ranks.js?v=1.1.14");
  ...
  async recordWin() {
    ...
    try {
      ...
      if (!this.isReplay && !isAlreadyWon) {
        ...
        const peaksErrors = this.state.stats?.peaksErrors || 0;
        const { calculateTimeBonus } = await import("./ranks.js?v=1.1.14");
      ...
      const { saveUserStats } = await import("./db.js?v=1.1.14");
      const { stopTimer } = await import("./timer.js?v=1.1.14");
      stopTimer();
      const { getDailySeed } = await import("./utils/random.js?v=1.1.14");
      const user = await import("./auth.js?v=1.1.14").then((m) => m.getCurrentUser());
      if (user && !user.isAnonymous) {
        const { getJigsudoDateString, getJigsudoYearMonth } =
          await import("./utils/time.js?v=1.1.14");
        const today = getJigsudoDateString();
