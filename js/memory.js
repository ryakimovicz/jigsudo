...
} from "./jigsaw.js?v=1.1.15";
import { provideHint as provideSudokuHint } from "./sudoku.js?v=1.1.15";
import { providePeaksHint } from "./peaks.js?v=1.1.15";
import { provideSearchHint } from "./search.js?v=1.1.15";
import { gameManager } from "./game-manager.js?v=1.1.15";
import { CONFIG } from "./config.js?v=1.1.15";
import { startTimer } from "./timer.js?v=1.1.15";
import { updateTexts } from "./i18n.js?v=1.1.15";
import { cleanupVictoryUI } from "./ui.js?v=1.1.15";
import { stopVictoryAnimations, debugSolveCode, resumeCodeState } from "./code.js?v=1.1.15";
import { resumeSudokuState } from "./sudoku.js?v=1.1.15";
import { resumePeaksState } from "./peaks.js?v=1.1.15";
...
export async function resumeToStage(stage) {
  ...
  if (stage === "jigsaw") {
    const m = await import("./jigsaw.js?v=1.1.15");
    m.resumeJigsawState();
    m.transitionToJigsaw();
  } else if (stage === "sudoku") {
    const jigsaw = await import("./jigsaw.js?v=1.1.15");
    jigsaw.resumeJigsawState();
    const sudoku = await import("./sudoku.js?v=1.1.15");
    sudoku.resumeSudokuState();
    sudoku.transitionToSudoku();
  } else if (stage === "peaks") {
    const jigsaw = await import("./jigsaw.js?v=1.1.15");
    jigsaw.resumeJigsawState();
    const peaks = await import("./peaks.js?v=1.1.15");
    peaks.transitionToPeaks();
  } else if (stage === "search") {
    const jigsaw = await import("./jigsaw.js?v=1.1.15");
    jigsaw.resumeJigsawState();
    const search = await import("./search.js?v=1.1.15");
