...
} from "./jigsaw.js?v=1.3.10";
import { provideHint as provideSudokuHint } from "./sudoku.js?v=1.3.10";
import { providePeaksHint } from "./peaks.js?v=1.3.10";
import { provideSearchHint } from "./search.js?v=1.3.10";
import { gameManager } from "./game-manager.js?v=1.3.10";
import { CONFIG } from "./config.js?v=1.3.10";
import { startTimer } from "./timer.js?v=1.3.10";
import { translations } from "./translations.js?v=1.3.10";
import { getCurrentLang, updateTexts } from "./i18n.js?v=1.3.10";
import { cleanupVictoryUI, updateLevelTitle } from "./ui.js?v=1.3.10";
import { stopVictoryAnimations, debugSolveCode, resumeCodeState } from "./code.js?v=1.3.10";
import { resumeSudokuState } from "./sudoku.js?v=1.3.10";
import { resumePeaksState } from "./peaks.js?v=1.3.10";
...
export async function resumeToStage(stage) {
  ...
  if (stage === "jigsaw") {
    const m = await import("./jigsaw.js?v=1.3.10");
    m.resumeJigsawState();
    m.transitionToJigsaw();
  } else if (stage === "sudoku") {
    const jigsaw = await import("./jigsaw.js?v=1.3.10");
    jigsaw.resumeJigsawState();
    const sudoku = await import("./sudoku.js?v=1.3.10");
    sudoku.resumeSudokuState();
    sudoku.transitionToSudoku();
  } else if (stage === "peaks") {
    const jigsaw = await import("./jigsaw.js?v=1.3.10");
    jigsaw.resumeJigsawState();
    const peaks = await import("./peaks.js?v=1.3.10");
    peaks.transitionToPeaks();
  } else if (stage === "search") {
    const jigsaw = await import("./jigsaw.js?v=1.3.10");
    jigsaw.resumeJigsawState();
    const search = await import("./search.js?v=1.3.10");
