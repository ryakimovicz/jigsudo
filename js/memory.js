...
} from "./jigsaw.js?v=1.4.0";
import { provideHint as provideSudokuHint } from "./sudoku.js?v=1.4.0";
import { providePeaksHint } from "./peaks.js?v=1.4.0";
import { provideSearchHint } from "./search.js?v=1.4.0";
...
export async function resumeToStage(stage) {
  ...
  if (stage === "jigsaw") {
    const m = await import("./jigsaw.js?v=1.4.0");
    m.resumeJigsawState();
    m.transitionToJigsaw();
  } else if (stage === "sudoku") {
    const jigsaw = await import("./jigsaw.js?v=1.4.0");
    jigsaw.resumeJigsawState();
    const sudoku = await import("./sudoku.js?v=1.4.0");
    sudoku.resumeSudokuState();
    sudoku.transitionToSudoku();
  } else if (stage === "peaks") {
    const jigsaw = await import("./jigsaw.js?v=1.4.0");
    jigsaw.resumeJigsawState();
    const peaks = await import("./peaks.js?v=1.4.0");
    peaks.transitionToPeaks();
  } else if (stage === "search") {
    const jigsaw = await import("./jigsaw.js?v=1.4.0");
    jigsaw.resumeJigsawState();
    const search = await import("./search.js?v=1.4.0");
