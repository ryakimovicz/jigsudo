...
export const router = {
  ...
  async handleRoute() {
    ...
    if (baseRoute === "#admin") {
      const { isAdmin } = await import("./auth.js?v=1.3.5");
      if (!isAdmin()) {
  ...
  updateView(activeId, baseRoute, params) {
    ...
    if (footer) {
      if (isGame) {
        ...
        const cleanup = async () => {
          const { cleanupVictoryUI } = await import("./ui.js?v=1.3.5");
          const { stopVictoryAnimations } = await import("./code.js?v=1.3.5");
          cleanupVictoryUI();
    ...
    if (activeId === "admin-section") {
      import("./admin.js?v=1.3.5").then((mod) => {
        mod.initAdmin();
        mod.showAdminPanel();
      });
    } else {
      import("./admin.js?v=1.3.5").then((mod) => mod.hideAdminPanel());
    }
