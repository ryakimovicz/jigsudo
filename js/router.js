...
export const router = {
  ...
  async handleRoute() {
    ...
    if (baseRoute === "#admin") {
      const { isAdmin } = await import("./auth.js?v=1.4.3");
      if (!isAdmin()) {
  ...
  async updateView(activeId, baseRoute, params) {
    ...
    if (footer) {
      if (hideFooter) {
        ...
        const cleanup = async () => {
          const { cleanupVictoryUI } = await import("./ui.js?v=1.4.3");
          const { stopVictoryAnimations } = await import("./code.js?v=1.4.3");
          cleanupVictoryUI();
    ...
    if (activeId === "admin-section") {
      import("./admin.js?v=1.4.3").then((mod) => {
        mod.initAdmin();
        mod.showAdminPanel();
      });
    } else {
      import("./admin.js?v=1.4.3").then((mod) => mod.hideAdminPanel());
    }
