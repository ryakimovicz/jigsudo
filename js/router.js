...
export const router = {
  ...
  async handleRoute() {
    ...
    if (baseRoute === "#admin") {
      const { isAdmin } = await import("./auth.js?v=1.4.5");
      if (!isAdmin()) {
  ...
  async updateView(activeId, baseRoute, params) {
    ...
    if (footer) {
      if (hideFooter) {
        ...
        const cleanup = async () => {
          const { cleanupVictoryUI } = await import("./ui.js?v=1.4.5");
          const { stopVictoryAnimations } = await import("./code.js?v=1.4.5");
          cleanupVictoryUI();
    ...
    if (activeId === "admin-section") {
      import("./admin.js?v=1.4.5").then((mod) => {
        mod.initAdmin();
        mod.showAdminPanel();
      });
    } else {
      import("./admin.js?v=1.4.5").then((mod) => mod.hideAdminPanel());
    }
