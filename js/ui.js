...
import { getRankData } from "./ranks.js?v=1.1.8";
import { formatJigsudoDate } from "./utils/time.js?v=1.1.8";
...
export async function showVictorySummary(stats, isHome = false) {
  ...
  if (descEl) {
    ...
    const { updateTexts } = await import("./i18n.js?v=1.1.8");
    updateTexts();
  }
  ...
  if (btnHome) {
    btnHome.onclick = async () => {
      ...
      if (!isHome) {
        ...
        try {
          const { clearRankingCache } = await import("./ranking.js?v=1.1.8");
          clearRankingCache();
        } catch (err) {
