...
import { db } from "./firebase-config.js?v=1.4.13";
import {
  collection,
  query,
  orderBy,
...
const dbMod = await import("./db.js?v=1.4.13");
...
export async function fetchRankings(forceRefresh = false) {
  ...
  const { getJigsudoDateString } = await import("./utils/time.js?v=1.4.13");
  ...
  const rankings = {
    daily: await getTopRankings(
      ...
      10,
      user,
      (await import("./db.js?v=1.4.13")).getUserRank,
      "lastDailyUpdate",
      today,
    ),
