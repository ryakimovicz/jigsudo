...
const dbMod = await import("./db.js?v=1.4.8");
import { getCurrentUser } from "./auth.js?v=1.4.8";
import { getRankData, SCORING } from "./ranks.js?v=1.4.8";
...
export async function fetchRankings(forceRefresh = false) {
  ...
  const { getJigsudoDateString } = await import("./utils/time.js?v=1.4.8");
  ...
  const rankings = {
    daily: await getTopRankings(
      ...
      10,
      user,
      (await import("./db.js?v=1.4.8")).getUserRank,
      "lastDailyUpdate",
      today,
    ),
