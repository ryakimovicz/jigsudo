...
import { db } from "./firebase-config.js?v=1.3.1";
import {
  collection,
  query,
  orderBy,
...
const dbMod = await import("./db.js?v=1.3.1");
import { getCurrentUser } from "./auth.js?v=1.3.1";
import { getRankData, SCORING } from "./ranks.js?v=1.3.1";
...
export async function fetchRankings(forceRefresh = false) {
  ...
  const { getJigsudoDateString } = await import("./utils/time.js?v=1.3.1");
  ...
  const rankings = {
    daily: await getTopRankings(
      ...
      10,
      user,
      (await import("./db.js?v=1.3.1")).getUserRank,
      "lastDailyUpdate",
      today,
    ),
