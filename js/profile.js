import { getCurrentUser, logoutUser } from "./auth.js?v=1.4.20";
import { getCurrentLang, updateTexts, getI18n } from "./i18n.js?v=1.4.20";
import { translations } from "./translations.js?v=1.4.20";
import { gameManager } from "./game-manager.js?v=1.4.20";
import { getRankData, calculateRP } from "./ranks.js?v=1.4.20";
import { formatTime } from "./ui.js?v=1.4.20";
import { getJigsudoDate, formatJigsudoDate, getJigsudoDateString } from "./utils/time.js?v=1.4.20";
import { fetchPuzzleIndex } from "./history.js?v=1.4.20";
...
function _showProfileUI(requestedUsername, skipInitialUpdate = false) {
  ...
  const startPolling = async () => {
    ...
    verificationInterval = setInterval(async () => {
      const { refreshUserStatus } = await import("./auth.js?v=1.4.20");
  ...
  import("./auth.js?v=1.4.20").then((mod) => {
...
export async function updateProfileData(targetUsername = activeProfileName) {
  ...
  if (!isOwnProfile) {
    ...
    try {
      const { getPublicUserByUsername } = await import("./db.js?v=1.4.20");
      ...
      if (publicData) {
        if (publicData.isPrivate) {
          ...
          const { getPublicUserHistory } = await import("./db.js?v=1.4.20");
          ...
          if (currentFavBtn) {
            ...
            newFavBtn.addEventListener("click", async () => {
              const { toggleFavorite } = await import("./db.js?v=1.4.20");
    ...
    verificationBanner.classList.toggle("hidden", !showBanner);
      ...
      if (resendBtn && !resendBtn.dataset.listenerAttached) {
        resendBtn.onclick = async () => {
          const { resendVerification } = await import("./auth.js?v=1.4.20");
...
function renderCalendar(history = {}, ownHistory = null, fName = "Ellos") {
  ...
  try {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      import("./history.js?v=1.4.20").then(mod => {
...
async function handleShareStats() {
  ...
  try {
    ...
    if (user && !user.isAnonymous) {
      try {
        const { fetchLatestUserData } = await import("./db.js?v=1.4.20");
