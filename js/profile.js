import { getCurrentUser, logoutUser } from "./auth.js?v=1.1.16";
import { getCurrentLang, updateTexts } from "./i18n.js?v=1.1.16";
import { translations } from "./translations.js?v=1.1.16";
import { gameManager } from "./game-manager.js?v=1.1.16";
import { getRankData, calculateRP } from "./ranks.js?v=1.1.16";
import { formatTime } from "./ui.js?v=1.1.16";
import { getJigsudoDate, formatJigsudoDate, getJigsudoDateString } from "./utils/time.js?v=1.1.16";
import { fetchPuzzleIndex } from "./history.js?v=1.1.16";
...
function _showProfileUI(requestedUsername) {
  ...
  const startPolling = async () => {
    ...
    verificationInterval = setInterval(async () => {
      const { refreshUserStatus } = await import("./auth.js?v=1.1.16");
  ...
  import("./auth.js?v=1.1.16").then((mod) => {
  ...
  import("./sidebar.js?v=1.1.16").then((mod) => {
...
export async function updateProfileData(targetUsername = activeProfileName) {
  ...
  if (!isOwnProfile) {
    ...
    try {
      const { getPublicUserByUsername } = await import("./db.js?v=1.1.16");
    ...
    verificationBanner.classList.toggle("hidden", !showBanner);
      ...
      if (resendBtn && !resendBtn.dataset.listenerAttached) {
        resendBtn.onclick = async () => {
          const { resendVerification } = await import("./auth.js?v=1.1.16");
...
async function handleShareStats() {
  ...
  try {
    ...
    if (user && !user.isAnonymous) {
      try {
        const { fetchLatestUserData } = await import("./db.js?v=1.1.16");
