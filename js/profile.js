import { getCurrentUser, logoutUser } from "./auth.js?v=1.3.6";
import { getCurrentLang, updateTexts } from "./i18n.js?v=1.3.6";
import { translations } from "./translations.js?v=1.3.6";
import { gameManager } from "./game-manager.js?v=1.3.6";
import { getRankData, calculateRP } from "./ranks.js?v=1.3.6";
import { formatTime } from "./ui.js?v=1.3.6";
import { getJigsudoDate, formatJigsudoDate, getJigsudoDateString } from "./utils/time.js?v=1.3.6";
import { fetchPuzzleIndex } from "./history.js?v=1.3.6";
...
function _showProfileUI(requestedUsername) {
  ...
  const startPolling = async () => {
    ...
    verificationInterval = setInterval(async () => {
      const { refreshUserStatus } = await import("./auth.js?v=1.3.6");
  ...
  import("./auth.js?v=1.3.6").then((mod) => {
  ...
  import("./sidebar.js?v=1.3.6").then((mod) => {
...
export async function updateProfileData(targetUsername = activeProfileName) {
  ...
  if (!isOwnProfile) {
    ...
    try {
      const { getPublicUserByUsername } = await import("./db.js?v=1.3.6");
    ...
    verificationBanner.classList.toggle("hidden", !showBanner);
      ...
      if (resendBtn && !resendBtn.dataset.listenerAttached) {
        resendBtn.onclick = async () => {
          const { resendVerification } = await import("./auth.js?v=1.3.6");
...
function renderCalendar(history = {}) {
  ...
  try {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      import("./history.js?v=1.3.6").then(mod => {
