...
import { startDailyGame } from "./home.js?v=1.3.1";
import { router } from "./router.js?v=1.3.1";
import { CONFIG } from "./config.js?v=1.3.1";

import { getJigsudoDate } from "./utils/time.js?v=1.3.1";
...
export function initHistory() {
  ...
  // v1.4.7: Disabled for Basic Edition
  /*
...
export async function updateHistoryUI() {
  ...
  let availableDates = CONFIG.isBasicEdition ? ["2026-04-15"] : await fetchPuzzleIndex();
  puzzleExistsCache = {};
  ...
  // 2. Local History check (no Firestore in demo)
  let monthHistory = {};
  const localStatsStr = localStorage.getItem("jigsudo_user_stats");
  if (localStatsStr) {
    try {
      const stats = JSON.parse(localStatsStr);
      if (stats.history) {
        // Filter history to current month or just pass the whole object
        // since renderHistoryCalendar only checks relevant date strings
        monthHistory = stats.history;
      }
    } catch (e) {}
  }
...
function renderHistoryCalendar(history = {}) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    // Single Puzzle restriction for Basic Edition
    const demoDate = "2026-04-15";
    const isDemoDate = dateStr === demoDate;

    if (isFuture || !exists || !isDemoDate) {
      dayEl.classList.add("disabled");
      dayEl.style.opacity = "0.2";
      dayEl.style.pointerEvents = "none";
      if (!isDemoDate) dayEl.style.filter = "grayscale(1)";
    } else {
      // Demo Date logic
      if (isToday && !isCompleted) {
        dayEl.style.opacity = "0.4"; // More muted if not playable here
      ...
      if (dayData) {
        // v1.9.5: Infer flags from available dayData (status, original, played)
        const isOriginalWon = dayData.originalWon || (dayData.original !== undefined);
        const isStartedOnly = !isOriginalWon && (dayData.hasOriginal || dayData.played === true);

        // 1. BACKGROUND COLORS (Original performance anchor)
        if (isOriginalWon) {
          // Green: Won on the original day
          dayEl.classList.add("win");
          dayEl.style.cursor = "pointer"; // Always clickable if won
          dayEl.style.opacity = "1";
        } else if (isStartedOnly) {
          // Yellow: Started but not finished on the original day
          dayEl.classList.add("loss");
          // If it's today and started but not won, still not clickable from history per request
          if (isToday) {
            dayEl.style.cursor = "default";
...
function showHistoryTooltip(e, data, dateStr, isMobile = false) {
  ...
  if (!data || (!data.original && !data.best)) {
    html += `<div style="color: var(--text-muted); font-size: 0.8rem; padding: 10px 0; text-align: center;">
      ${t.history_no_stats || "Sin estadísticas registradas"}
    </div>`;
  } else {
    const lang = getCurrentLang() || "es";
    ...
    if (data.original) {
      ...
      html += `<div class="tooltip-section">
        ...
        <div class="tooltip-grid">
          ...
          <span class="tooltip-value highlight">${numFmt.format(o.score || 0)} RP</span>
    ...
    if (data.best) {
      ...
      html += `<div class="tooltip-section">
        ...
        <div class="tooltip-grid">
          <span class="tooltip-label">${lang === 'es' ? 'Puntaje' : 'Score'}:</span>
          <span class="tooltip-value highlight">${numFmt.format(b.score || 0)} RP</span>
          <span class="tooltip-label">${lang === 'es' ? 'Tiempo' : 'Time'}:</span>
